...
async function getSigningKey() {
  ...
  if (process.env.SESSION_SECRET) {
    ...
    var hash = await globalThis.crypto.subtle.digest('SHA-256', new TextEncoder().encode(process.env.ADMIN_PASSWORD || 'shutterfolio-insecure-fallback'));
    secret = hex(hash);
  }
  cachedKey = await globalThis.crypto.subtle.importKey('raw', new TextEncoder().encode(secret), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign', 'verify']);
  return cachedKey;
}
...
async function generateSessionToken(username, password) {
  if (username === process.env.ADMIN_USERNAME && password === process.env.ADMIN_PASSWORD) {
    ...
    var sig = hex(await globalThis.crypto.subtle.sign('HMAC', key, new TextEncoder().encode(encoded)));
    return encoded + '.' + sig;
  }
  return null;
}
...
async function verifySession(cookieValue) {
  ...
  try {
    ...
    var sig = parts[1];
    var key = await getSigningKey();
    var valid = await globalThis.crypto.subtle.verify('HMAC', key, fromHex(sig), new TextEncoder().encode(encoded));
